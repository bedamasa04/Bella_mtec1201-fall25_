...
function preload()
{
  ...
  pointerGrab = loadImage("images/fist.png");
...
if (mouseX > 40 && mouseX < 120 && mouseY > 280 && mouseY < 360)
{
  ...
  image(pointer, 80, 380,  pointer.width / 15, pointer.height / 15);
  }
 else if (keyIsDown(RIGHT_ARROW))
 {
  ...
  text("Erase", 450, 400);
  image(pointer, 400, 380,  pointer.width / 15, pointer.height / 15);
 }
 // I WANTED ADD A RESULT IF YOU PRESSED EITHER BUTTON but nothign is happening when I add &&
 else if (keyIsDown(LEFT_ARROW) && keyIsDown(ENTER))
 {
  image(matt, -660, -700, matt.width * 4, matt.width * 3);
fill(255,0,10);
stroke(0);
textAlign(CENTER);
text("You can't.", width / 2, height / 2);
 }
...
if (mouseIsPressed)
{
  image(pointerGrab, mouseX, mouseY,  pointerGrab.width / 1.75, pointerGrab.height / 1.75);
}
else
{
  image(pointer, mouseX, mouseY, pointer.width / 15, pointer.height / 15);
